...
var user = User(name: "palak", age: 20, height: 154, weight: 54, activityLevel: 8)
print(user.name)
print(user.age, user.height, user.weight, user.activityLevel)
/*:
...
struct Distance{
    ...
    init(feets:Double){
        self.feet = feets
        self.metres = feets/3.38084
    }
}

//:  Now create an instance of `Distance` called `mile`. Use the initializer for meters to set the distance to 1600. Print out the property for feet and verify that it is equal to 5249.344.
var mile = Distance(metre: 1600)
print(mile)
//:  Now create another instance of `Distance` and give it some other distance. Ensure that both properties are set correctly.
var distance = Distance(feets: 5)
print(distance)
/*:
